            ...
            <div class="container-publication">
                ...
                <div class="header-publication">
                    ...
                    <div class="publication-info">
                        ...
                        <p class="publication-info__delete" @click="deletePublication()">X</p>     
                    </div>          
                </div>     
                ...
                <div class="commitment-section">
                    <div v-if="getPublicationsLikedFromLocalStorage() && getPublicationsLikedFromLocalStorage().includes(publication.id)" class="commitment-section-row">
                        <img @click="onLikePublication()" class="commitment-section-row__icon" src="../assets/heart-solid.svg"><span>{{ likes }}</span> 
                    </div>
                    <div v-else class="commitment-section-row">
                        <img @click="onLikePublication()" class="commitment-section-row__icon" src="../assets/heart-regular.svg"><span>{{ likes }}</span> 
                    </div>
                ...
                <div class="commentary-section">
                    ...
                    <div :key="index" v-for="(comment, index) in comments" class="publication-list-commentary-container">
                        <div class=profile-pic-section>
                            <img class="profile-pic-section__author-pic" :src="comment.profile_pic_user">
                        </div>
                        <div class="user-commentary-container">
                            <div class="commentary-info">
                                <p class="commentary-info__author-name">{{ comment.first_name_user }} {{ comment.last_name_user }}</p>
                                <p class="commentary-info__date">{{ comment.date_comment_fr }}</p>
                                <p class="commentary-info__delete" @click="deleteOneComment(comment.id)">X</p>  
                            </div> 
                    ...
                    <div class="new-commentary-container">
                        ...
                        <div class="new-commentary">
                            <textarea v-model="addNewComment" class="new-commentary__input-text" name="" placeholder="Ecrivez un commentaire..."></textarea><span><button @click="publishNewComment(publication.id)" class="button">Publier</button></span>
                        </div>
                    </div>
                </div>
            </div>
...
export default {
    ...
    mounted(){
        ...
        .then(res => {
            ...
            this.likes = res.data.publication[0].likes
        }) 
        .catch(error => {
        ...
        // Get publications liked by the user from Local Storage
        axios.get(`http://localhost:3000/api/publication/likes/user/${this.getUserIdFromLocalStorage()}`,{
            headers: { Authorization: `Bearer ${this.getTokenFromLocalStorage()}` }
        })
        .then(response => {

            const arrPublicationsId = []
            
            for(let i = 0; i < response.data.length; i++){
                arrPublicationsId.push(response.data[i].publication_id)
            }

            localStorage.setItem('groupomania_publicationsLiked', JSON.stringify(arrPublicationsId)) 
    ...
    methods:{
        ...
        deleteOneComment(commentId){
            ...
            axios.delete(`http://localhost:3000/api/comment-publication/${commentId}`, 
            { headers: { Authorization: `Bearer ${this.getTokenFromLocalStorage()}`}})
            .then(res => {
                console.log(res)
        ...
        onLikePublication(){
            ...
            if(!this.getPublicationsLikedFromLocalStorage()){
                ...
                axios.post(`http://localhost:3000/api/publication/likes/${this.publicationId}`, 
                { like: 1 },
                { headers: { Authorization: `Bearer ${this.getTokenFromLocalStorage() }`}})
                .then(res => {
                    console.log(res)
